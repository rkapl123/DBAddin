...
err1:
    ...
    Public Function saveFilterSettings(ws As Worksheet) As Object
        ...
        DecSep = theHostApp.International(XlApplicationInternational.xlDecimalSeparator)

        On Error GoTo err1
        With ws.AutoFilter.Filters
            ReDim filterArray(0 To .Count, 0 To 3)
            For f As Long = 1 To .Count
                With .Item(f)
                    If .On Then
                        Dim dummyBool As Boolean
                        ' why does Microsoft not do proper internationalization????
                        ' need to check by hand whether we need to convert "comma" to "dot"
                        checkNum = Replace(Replace(Replace(Replace(.Criteria1, "<", vbNullString), "*", vbNullString), ">", vbNullString), "=", vbNullString)
                        filterArray(f, 1) = .Criteria1
                        If IsNumeric(checkNum) Then
                            filterArray(f, 1) = IIf(DecSep <> ".", Replace(.Criteria1, DecSep, "."), .Criteria1)
                        ElseIf IsDate(checkNum) Then
                            filterArray(f, 1) = Replace(.Criteria1, checkNum, CDate(checkNum).ToOADate())
                            ' special case for single dates, need to insert another criterion
                            If Left$(.Criteria1, 1) = "=" And .Operator = 0 Then
                                filterArray(f, 1) = Replace(filterArray(f, 1), "=", ">=")
                                filterArray(f, 2) = 1
                                filterArray(f, 3) = "<=" & CDate(checkNum).ToOADate()
                            End If
                            ' Booleans are also only working in US versions...
                        Else
                            On Error Resume Next
                            dummyBool = CBool(checkNum)
                            If Err.Number = 0 Then filterArray(f, 1) = Replace(.Criteria1, checkNum, IIf(CBool(checkNum), "True", "False"))
                            Err.Clear()
                        End If

                        If .Operator <> 0 Then
                            filterArray(f, 2) = .Operator
                            checkNum = Replace(Replace(Replace(Replace(.Criteria2, "<", vbNullString), "*", vbNullString), ">", vbNullString), "=", vbNullString)
                            filterArray(f, 3) = .Criteria2
                            If IsNumeric(checkNum) Then
                                filterArray(f, 3) = IIf(DecSep <> ".", Replace(.Criteria2, DecSep, "."), .Criteria2)
                            ElseIf IsDate(checkNum) Then
                                filterArray(f, 3) = Replace(.Criteria2, checkNum, CDate(checkNum).ToOADate())
                            Else
                                On Error Resume Next
                                dummyBool = CBool(checkNum)
                                If Err.Number = 0 Then filterArray(f, 3) = Replace(.Criteria2, checkNum, IIf(CBool(checkNum), "True", "False"))
                                Err.Clear()
                            End If
                        End If
                    End If
        ...
        End With
        On Error Resume Next
        saveFilterSettings = filterArray
        Exit Function
err1:
    ...
    Public Function restoreFilterSettings(filteredRange As Object, filterArray As Object) As Boolean
        ...
        filteredRange.AutoFilter(Field:=1)
        Dim f As Long
        For f = 1 To UBound(filterArray, 1)
            If Not filterArray(f, 1) Is ExcelEmpty.Value Then
                If filterArray(f, 2) Then
                    filteredRange.AutoFilter(Field:=f, Criteria1:=filterArray(f, 1), Operator:=filterArray(f, 2), Criteria2:=filterArray(f, 3))
                Else
                    filteredRange.AutoFilter(Field:=f, Criteria1:=filterArray(f, 1))
                End If
            End If
        Next
